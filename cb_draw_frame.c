...
/*   Updated: 2020/08/03 06:34:28 by jnannie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			cb_draw_line(t_cbdata *cbdata, int x, int line_start, int line_end, t_cbimage *texture, double texture_draw_start, int texX)
{
	...
	texture_step_y = (texture->height - 2 * texture_draw_start) / (line_end - line_start + 1);
	...
	while (y <= line_end)
	{
		texture_x = (int)(texture_draw_start + tex_y * texture_step_y) * texture->size_line / 4 + texX;
		if ((((int *)(texture->image))[texture_x] & 0x00FFFFFF) != 0)
			((int *)(cbdata->frame->image))[x] = ((int *)(texture->image))[texture_x];
		else
			((int *)(cbdata->frame->image))[x] = 0x00FFFFFF;
...
void			cb_draw_frame(t_cbdata *cbdata)
{
	...
	cb_print_floor_and_ceilling(cbdata);
		...
		{
			ray.ray_x = cbdata->dir_x + cbdata->plane_x * (-1 + (2.0 / cbdata->frame->width) * x);
			ray.ray_y = cbdata->dir_y + cbdata->plane_y * (-1 + (2.0 / cbdata->frame->width) * x);
			ray.map_x = (int)(cbdata->pos_x);
			ray.map_y = (int)(cbdata->pos_y);
			if (ray.ray_x < 0)
				ray.dist_x = (cbdata->pos_x - ray.map_x) * cb_delta_dist(ray.ray_x);
			else
				ray.dist_x = (ray.map_x + 1.0 - cbdata->pos_x) * cb_delta_dist(ray.ray_x);
			if (ray.ray_y < 0)
				ray.dist_y = (cbdata->pos_y - ray.map_y) * cb_delta_dist(ray.ray_y);
			else
				ray.dist_y = (ray.map_y + 1.0 - cbdata->pos_y) * cb_delta_dist(ray.ray_y);
			while (cbdata->map[ray.map_y][ray.map_x] != '1')
				if (ray.dist_x < ray.dist_y)
				{
					ray.dist_x += cb_delta_dist(ray.ray_x);
					ray.map_x += cb_direction(ray.ray_x);
					ray.wall_side = 0;
				}
				else
				{
					ray.dist_y += cb_delta_dist(ray.ray_y);
					ray.map_y += cb_direction(ray.ray_y);
					ray.wall_side = 1;
				}
			if (ray.wall_side == 0)
				ray.perp_dist = (ray.map_x - cbdata->pos_x + (1 - cb_direction(ray.ray_x)) / 2) / ray.ray_x;
			else
				ray.perp_dist = (ray.map_y - cbdata->pos_y + (1 - cb_direction(ray.ray_y)) / 2) / ray.ray_y;

			lineHeight = (int)(cbdata->frame->height / ray.perp_dist);


			drawStart = (-lineHeight + cbdata->frame->height) / 2;

			if (ray.wall_side == 0 && ray.ray_x > 0)
				texture = cbdata->we_texture;
			else if (ray.wall_side == 0 && ray.ray_x < 0)
				texture = cbdata->ea_texture;
			else if (ray.wall_side == 1 && ray.ray_y > 0)
			...
			if (ray.wall_side == 0) wallX = cbdata->pos_y + ray.perp_dist * ray.ray_y;
			else           wallX = cbdata->pos_x + ray.perp_dist * ray.ray_x;
			wallX -= floor((wallX));

			int texX = (int)(wallX * (double)(texture->width));
			if(ray.wall_side == 0 && ray.ray_x < 0) texX = texture->width - texX - 1;
			if(ray.wall_side == 1 && ray.ray_y > 0) texX = texture->width - texX - 1;

			cb_draw_line(cbdata, x, drawStart, drawEnd, texture, texture_draw_start, texX);

			x++;
			ZBuffer[x] = ray.perp_dist;
		}
