...
/*   Updated: 2020/08/03 01:32:44 by jnannie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cb_move(t_cbdata *cbdata, double dir_x, double dir_y, int dir)
{
	...
	dx = dir_x < 0 ? -1 : 1;
	dy = dir_y < 0 ? -1 : 1;
	if (cbdata->map[(int)(cbdata->pos_y)][(int)(cbdata->pos_x +/* dir_x * cbdata->moveSpeed * dir +*/ cbdata->moveSpeed * dir * dx *CB_INDENTATION)] != '1')// * CB_INDENTATION)] == '0')
		cbdata->pos_x += dir_x * cbdata->moveSpeed * dir;
	if (cbdata->map[(int)(cbdata->pos_y + /*dir_y * cbdata->moveSpeed * dir +*/ cbdata->moveSpeed * dir * dy * CB_INDENTATION)][(int)(cbdata->pos_x)] != '1')// * CB_INDENTATION)] == '0')
...
int		cb_key_release_hook(int keycode, void *cbdata)
{
	if (keycode == CB_LEFT)
		...
		((t_cbdata *)cbdata)->key_w = 0;
	else if (keycode == CB_UP)
		((t_cbdata *)cbdata)->key_up = 0;
	else if (keycode == CB_CAPITAL_S || keycode == CB_SMALL_S)
		((t_cbdata *)cbdata)->key_s = 0;
	else if (keycode == CB_DOWN)
	...
	if (!((t_cbdata *)cbdata)->key_rot_left && !((t_cbdata *)cbdata)->key_rot_right &&
		...
		!((t_cbdata *)cbdata)->key_w && !((t_cbdata *)cbdata)->key_s)
